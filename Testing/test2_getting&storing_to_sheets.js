...
async function writeSheet(sheets, rowIndex, screenshotFilename) {
...
async function scrapeGoogle(page, searchQuery, targetDomain) {
  console.log(
    `Starting scrape for query: "${searchQuery}", target domain: "${targetDomain}"`
  );
  ...
  try {
    ...
    // Find snippet containing target domain
...
(async () => {
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      try {
        const screenshotName = await scrapeGoogle(
          page,
          searchQuery,
          targetDomain
        );
        await writeSheet(sheets, i + 1, screenshotName);
      } catch (e) {
