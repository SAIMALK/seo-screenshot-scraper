...
async function writeSheet(sheets, spreadsheetId, sheetName, rowIndex, value) {
...
async function uploadToGyazo(filePath, token) {
  if (!token) throw new Error("GYAZO_ACCESS_TOKEN not set");
...
async function scrapeGoogle(page, searchQuery, targetDomain, gyazoToken) {
  console.log(
  ...
  try {
    ...
    if (!targetDomain || targetDomain.trim() === "") {
      ...
      const gyazoUrl = await uploadToGyazo(screenshotPath, gyazoToken);
